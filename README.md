...
| **AI / ML** | **Python, FastAPI** | LLM 인터페이스, 임베딩 처리, RAG 파이프라인 구축 
 ...
 |
| **AI Tool** | **LangFlow, OpenAI API** | AI 워크플로우 설계 및 LLM(GPT) 연동 

 |
| **Frontend** | **React, Nginx** | 민원인/공무원 UI, 대시보드(Recharts), 정적 파일 서빙 
...
1. 
**Client:** 민원인(Web/Mobile) 및 공무원(Dashboard)이 Nginx(Reverse Proxy)를 통해 접속.


2. 
**API Gateway:** Nginx가 `/api/` 요청은 Spring Boot로, 정적 리소스는 React로 라우팅.
...
5. 
**Database:** 민원 데이터, 사용자 정보, 벡터 데이터(임베딩된 법령/사례) 저장.



---

## ✨ 4. 핵심 기능 및 기술적 상세 (Key Features)
...
* 
**결과 제공:** 상위 3개 추천 부서와 각각의 신뢰도(%)를 시각화하여 제공.
...
| 이름 | 역할 | 담당 업무 상세 |
| --- | --- | --- |
| **[본인 이름]** | Team Leader<br>

<br>Full Stack & AI | • **아키텍처 총괄:** Spring Boot + FastAPI MSA 구조 설계 및 Docker/GCP 인프라 구축<br>

<br>• **Backend:** 민원 처리 핵심 로직(JPA/QueryDSL), 인증 시스템 구현<br>

<br>• **AI Core:** RAG 파이프라인 설계, 임베딩/라우팅 알고리즘 최적화, 프롬프트 엔지니어링<br>

<br>• **Frontend:** 대시보드 시각화 및 공무원 관리자 페이지 개발 |
| **고상현** | Backend & AI | • 민원 데이터 전처리(Preprocessing) 및 벡터 DB 구축<br>

<br>• DBSCAN 군집화 알고리즘 구현 및 최적화 |
| **송병곤** | Frontend | • 사용자(민원인) 인터페이스 개발 및 UI/UX 디자인<br>

<br>• 반응형 웹 구현 및 API 연동 |
| **허진욱** | Backend | • 외부 데이터 수집(Web Scraping) 파이프라인 보조<br>
...
```bash
git clone https://github.com/your-repo/minwon-compass.git
cd minwon-compass
