...
async def retrieve_references(complaint_id: int, query: str = None) -> Dict[str, List]:
    ...
    if query is None:
        ...
        refined_query = await optimize_query(query)
        print(f"   -> 변환된 쿼리: {refined_query}")
        
        # 2. 임베딩 생성
        query_vec = await get_embedding(refined_query)
...
async def generate_rag_response(complaint_id: int, user_query: str = None) -> str:
    ...
    references = await retrieve_references(complaint_id, user_query)
